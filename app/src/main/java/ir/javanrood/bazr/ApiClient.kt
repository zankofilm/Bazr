...
class ApiClient(private val security:DeviceSecurity){
    ...
    /**
     * Executes an HTTP request with one short retry for transient DNS/network failures.
     * Persistent UnknownHost errors are converted to a clear diagnostic message instead
     * of exposing the raw OkHttp exception to the activation screen.
     */
    private fun executeWithNetworkRetry(request: Request): Response {
        ...
        when (last) {
            is UnknownHostException -> throw IOException(
                "DNS_ERROR: دامنه scooljavanrood.ir از شبکه این دستگاه پیدا نشد. اینترنت، Private DNS یا VPN را بررسی کنید و دوباره تلاش کنید.",
                last
            )
            is SocketTimeoutException -> throw IOException(
                "NETWORK_TIMEOUT: ارتباط با سرور زمان‌بر شد. اینترنت را بررسی کرده و دوباره تلاش کنید.",
                last
            )
            else -> throw last ?: IOException("NETWORK_ERROR")
        }
    }
    private fun postJson(endpoint:String,j:JSONObject,auth:Boolean=false):JSONObject{
        ...
        executeWithNetworkRetry(req).use{r-> val text=r.body?.string().orEmpty();val o=if(text.isBlank())JSONObject() else JSONObject(text);if(!r.isSuccessful)throw ApiException(r.code,o.optString("error","HTTP_${r.code}"));return o}
    }
    ...
    suspend fun sync():JSONObject=withContext(Dispatchers.IO){
        ...
        executeWithNetworkRetry(req).use{r->val t=r.body?.string().orEmpty();val o=JSONObject(t);if(!r.isSuccessful)throw ApiException(r.code,o.optString("error"));o}
    }
    ...
    suspend fun uploadEvidence(missionKey:String,file:File,mime:String,questionKey:String="",fileKind:String="document",capturedAt:String=""):JSONObject=withContext(Dispatchers.IO){
        ...
        executeWithNetworkRetry(req).use { r ->
    ...
    suspend fun downloadMobileFile(uploadId:Int):ByteArray=withContext(Dispatchers.IO){
        ...
        executeWithNetworkRetry(req).use{r->if(!r.isSuccessful)throw ApiException(r.code,"HTTP_${r.code}");r.body?.bytes()?:ByteArray(0)}
    }
