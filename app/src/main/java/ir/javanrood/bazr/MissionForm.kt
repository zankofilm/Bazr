...
fun MissionFormScreen(mission: MissionEntity, vm: BazrViewModel, onBack: () -> Unit, onCompleted: () -> Unit) {
    ...
    LaunchedEffect(mission.key) {
        ...
        vm.loadDraft(mission.key)?.let { draft ->
            runCatching {
                ...
                evidenceFromJson(root.optJSONObject("evidence")).forEach { (id, list) ->
                    val existing = list.filter { EvidenceStore.exists(it) }
                    missingEvidence += list.size - existing.size
                    if (existing.isNotEmpty()) evidence[id] = existing
                }
                if (missingEvidence > 0) {
                    validationMessage = "$missingEvidence فایل قدیمی روی گوشی پیدا نشد و از پیش‌نویس حذف شد."
    ...
    fun saveCurrentDraft(message: String = "پیش‌نویس روی همین گوشی ذخیره شد.") {
        if (generalNote.isNotBlank()) notes["general"] = generalNote else notes.remove("general")
        vm.saveDraft(mission, answers, notes, evidence)
