...
class BazrViewModel(app: Application) : AndroidViewModel(app) {
    ...
    fun activate(nationalId: String) {
        viewModelScope.launch {
            ...
            runCatching {
                ...
                val friendly = when {
                    raw.startsWith("DNS_ERROR:") -> raw.removePrefix("DNS_ERROR:").trim()
                    raw.startsWith("NETWORK_TIMEOUT:") -> raw.removePrefix("NETWORK_TIMEOUT:").trim()
                    else -> "فعال‌سازی انجام نشد: $raw"
                }
                _ui.value = _ui.value.copy(busy = false, message = friendly)
            }
        }
    }
    ...
    fun finalSubmit(
        ...
        viewModelScope.launch {
            ...
            // Persist the complete local snapshot before any network work starts.
            ...
            val submitResult = runCatching {
                ...
                val missing = allEvidence.filterNot { EvidenceStore.exists(it.second) }
                require(missing.isEmpty()) {
                    "این فایل‌ها روی گوشی پیدا نشدند: " + missing.joinToString("، ") { it.second.name }
                }
                ...
                if (allEvidence.isEmpty()) {
                    ...
                    allEvidence.forEachIndexed { index, (qid, item) ->
                        ...
                        val up = try {
                            api.uploadEvidence(mission.key, file, item.mime, qid, item.kind, EvidenceStore.capturedAt(item))
                        } catch (e: Exception) {
                            error("آپلود ${item.name} انجام نشد: ${e.message ?: "خطای نامشخص"}")
                        }
                        val fileId = up.optInt("file_id", 0)
                        require(fileId > 0) { "سرور شناسه معتبر برای ${item.name} برنگرداند" }
            ...
            pdfResult.onSuccess { pdf ->
                ...
                // Keep the local draft/evidence snapshot so the PDF can be regenerated later without losing attachments.
                db.drafts().save(DraftEntity(mission.key, local.toString(), finalPending = false))
